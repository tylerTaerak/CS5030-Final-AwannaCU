...
void writeVisiblePoints(int numRows, int numColumns, uint8_t radius, unsigned short* data);
...
void writeVisiblePoints(int numRows, int numColumns, uint8_t radius, unsigned short* data) {
  ...
  for (int y = 0; y < numRows; y++) {
      for (int x = 0; x < numColumns; x++) {
          ...
          fwrite(&visiblePoints, sizeof(uint32_t), 1, outFile); // Write the amount to the file.
      }
  }
